(ns fmuit.client.ui.common.forms
              ...
              (fn [{::uism/keys [state-map] :as env}]
                ...
                (let [FormClass (uism/actor-class env :actor/form)
                  ...
                  (if-not save-mutation
                    (log/info (str "Class " FormClass " must have a 'fmuit.client.common.forms/save-mutation"))
                    (-> env
                        (uism/assoc-aliased :busy? true)
             ...
             {::uism/handler
              (fn [env]
                (let [proceed (js/confirm "Really delete?")
                  ...
                  (cond
                    (not delete-mutation)
                    (log/error (str "Class " FormClass " must have a 'fmuit.client.common.forms/delete-mutation"))
       ...
       (fn [{::uism/keys [state-map] :as env}]
         ...
         (let [FormClass (uism/actor-class env :actor/form)
           ...
           (if-not create-mutation
             (log/info (str "Class " FormClass " must have a 'fmuit.client.common.forms/create-mutation"))
             (-> env
                 (uism/assoc-aliased :busy? true)
